...
保存まわりは**どこで失敗しても学習を止めない**。そのぶん、これまでは
黙って諦めるだけで、「毎回作り直しているのは仕様なのか故障なのか」が
外からは区別できなかった。次の2つで区別できるようにする。

- 失敗した理由を、実行ログとスクリプトプロパティ `AI_CACHE_LAST_TROUBLE` へ残す。
  記録に失敗しても本体の動きは変えない（例外を外へ出さない）
- Apps Script エディタから **`checkAiExplanationCache()`** を実行すると、
  実際のスプレッドシートを読んで次を報告する。**読むだけで1文字も書かない**
  1. `07_AI解説キャッシュ` があるか（無い＝一度も保存できていない）
  2. 何件残っているか
  3. 1件ずつ、**いま**使い回せるか。問題文・選択肢・登録解説を直すと署名が変わり、
     その解説は作り直しになる（古い内容を出し続けないための正しい動き）
  4. 保存したあとに、**同じ問題を同じ選択肢で答え直した回数**
     ＝再表示の出番が実際に何回あったか。ここが0なら、保存できていても
     再表示は起きようがない
- 出番が来ない代表例: 間違えた問題を解き直して**今度は正解した**とき。
  キーは `question_id` × `user_answer` なので、選んだ選択肢が変われば別あつかいになる。
  解説は「選んだ選択肢がなぜ違うのか」を含むため、ここは意図どおり
- 点検も本文の列（6列目）を行数ぶんまとめて読まない。短いキー列だけを読む
